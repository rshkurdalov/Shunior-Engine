...
bool http_request::setup(string_line<char8> source)
{
	clear();
	uint64 p = 0, e = 0;
	string str;
	...
	parser pr(source.addr + p, e - p, parse_mode::text, string_format::u8);
	pr >> str;
	...
	while(true)
	{
		...
		while(e < source.length && source.addr[e] != '\r') e++;
		if(e + 3 >= source.length || source.addr[e + 1] != '\n') return false;
		pr = parser(source.addr + p, e - p, parse_mode::text, string_format::u8);
		pr >> str;
